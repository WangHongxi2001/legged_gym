...
from .nezha_config import WheelLeggedRobotCfg
...
class Leg:
    def __init__(self, cfg: WheelLeggedRobotCfg.Leg, num_envs, device):
        ...
        self.end_x = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.end_y = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.end_x_dot = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.end_y_dot = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.L0 = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
    ...
    def Solve(self):
        self.end_x, self.end_y, self.L0, self.theta0 = self.Forward_Kinematics(
            self.theta1, self.theta2
        )
        dt = 0.001
        temp_end_x, temp_end_y, temp_L0, temp_theta0 = self.Forward_Kinematics(
            self.theta1 + self.theta1_dot * dt, self.theta2 + self.theta2_dot * dt
        )
    ...
    def VMC(self, F, T):
        ...
        t11 = self.l1 * torch.sin(self.theta0 - self.theta1) - self.l2 * torch.sin(
            self.theta1 + self.theta2 - self.theta0
        )

        t12 = self.l1 * torch.cos(self.theta0 - self.theta1) - self.l2 * torch.cos(
            self.theta1 + self.theta2 - self.theta0
        )
        t12 = t12 / self.L0
    ...
    def PD_Update(self, L0_reference, alpha_reference):
        F = (
            self.L0_kp * (L0_reference - self.L0)
            + self.L0_kd * (0 - self.L0_dot)
            + self.L0_ff
        )
        T = self.alpha_kp * (alpha_reference - self.alpha) + self.alpha_kd * (
            0 - self.alpha_dot
        )
        return self.VMC(F, T)
